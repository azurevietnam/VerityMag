  ...
  <ul>
    ...
    <li class="<?php echo ($key%2) ? "odd" : "even"; if(count($items)==$key+1) echo ' lastItem'; ?>">
      ...
      <a class="k2Avatar moduleItemAuthorAvatar" rel="author" href="<?php echo $item->authorLink; ?>">
                ...
                <?php
                    if(isset($item->authorAvatar)){
                        $src =$item->authorAvatar;  }
                    
                    if (!empty( $src)) {
                        $thumb_img = '<img style="width:'.$avatarWidth .'px;height:auto;" src="'.$src.'" alt="'.K2HelperUtilities::cleanHtml($item->author).'" />';
                    } else if ($is_placehold) {
                        $thumb_img = yt_placehold($placehold_size['k2_content_avatar'],$item->author,$item->author);
                    }	
                    echo $thumb_img;
                    ?>
			</a>
      ...
      <?php if($params->get('itemImage')): ?>
	      ...
	      <a class="moduleItemImage" href="<?php echo $item->link; ?>" title="<?php echo JText::_('K2_CONTINUE_READING'); ?> &quot;<?php echo K2HelperUtilities::cleanHtml($item->title); ?>&quot;">
             <?php
                 //var_dump($params->get('itemImage'));die;
							...
							if (!empty( $src)) {								
								...
								$thumb_img = yt_placehold($placehold_size['k2_content'],$item->title,$item->title);
							}	
							echo $thumb_img;
						?>
	      </a>
    ...
    <?php if ($key>=0):?>
        <div class="module-text"><?php echo ($key+1); ?></div>
    <?php endif; ?>
    <?php if($params->get('itemCategory') || $params->get('itemTitle')): ?>
        <div class="module-custom">
    <?php if($params->get('itemCategory')): ?>
      <div><h3><a class="moduleItemCategory" href="<?php echo $item->categoryLink; ?>"><?php echo $item->categoryname; ?></a></h3></div>
      <?php endif; ?>
...
<?php echo ($params->get('itemIntroTextWordLimit')>1) ? (K2HelperUtilities::wordLimit($item->introtext, $params->get('itemIntroTextWordLimit')-1,'')): $item->introtext; ?>
...
<?php endif; ?>
      ...
      <span class="moduleItemDateCreated"><?php echo JText::_('K2_WRITTEN_ON') ; ?> <?php echo JHTML::_('date', $item->created, JText::_('DATE_FORMAT_TEMPLATE')); ?></span>
      <?php endif; ?>
