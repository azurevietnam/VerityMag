...
$lang->load('com_finder', JPATH_SITE);
    ...
    $suffixid = rand() . time();
    $tag_id = 'mod_finder_searchword_' . $suffixid;
    $tag_form_id = 'mod-finder-searchform_' . $suffixid;
    
$suffix = $params->get('moduleclass_sfx');
$output = '<input type="text" name="q" id="' .$tag_id .'" class="mod-finder-searchword-id search-query input-medium" size="' . $params->get('field_size', 20) . '" value="' . htmlspecialchars(JFactory::getApplication()->input->get('q', '', 'string')) . '" />';

if ($params->get('show_label', 1))
{
	$label = '<label for="'.$tag_id.'" class="finder' . $suffix . '">' . $params->get('alt_label', JText::_('JSEARCH_FILTER_SUBMIT')) . '</label>';
...
jQuery(document).ready(function() {
	var value, searchword = jQuery('#$tag_id');
		...
		jQuery('#$tag_form_id').on('submit', function(e){
			e.stopPropagation();
...
if ($params->get('show_autosuggest', 1))
{
	...
	$script .= "
	var suggest = jQuery('#$tag_id').autocomplete({
...
<form id="<?php echo $tag_form_id; ?>" action="<?php echo JRoute::_($route); ?>" method="get" class="form-search">
	<div class="mod-finder-searchform-id finder<?php echo $suffix; ?>">
		<?php
