		...
		*/
		...
		public static function getAImages($item, $params, $prefix='imgcfg'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				$placeholder = 	 $params->get($prefix.'_placeholder_path', 'modules/mod_sj_basic_news/assets/images/nophoto.png');
				if (!empty($placeholder)){
					$placeholder = file_exists($placeholder) ? $placeholder :  $placeholder;
				}
				
				if ( count($images_path) == 0 && $params->get($prefix.'_placeholder', 1)==1 && !empty($placeholder)){
					$images_path[] = array('src'=> $placeholder, 'class'=>'placeholder');
				}
